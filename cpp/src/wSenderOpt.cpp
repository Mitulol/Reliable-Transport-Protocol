...
int main(int argc, char* argv[]) {
    ...
    // Individual timers for each in-flight packet
    std::unordered_map<uint32_t, std::chrono::steady_clock::time_point> packetTimers;

    // Track which packets have been ACKed
    std::unordered_set<uint32_t> ackedPackets;
    ...
    while (baseSeqNum < chunks.size()) {
        ...
        while (nextSeqNum < baseSeqNum + windowSize && nextSeqNum < chunks.size()) {
            ...
            packetTimers[nextSeqNum] = std::chrono::steady_clock::now();
        }
        ...
        if (ready > 0 && FD_ISSET(sockfd, &readfds)) {
            ...
            if (ackLen >= sizeof(PacketHeader)) {
                ...
                if (ntohl(ack->type) == 3) {
                    ...
                    if (ackedPackets.count(ackNum) == 0) {
                        ackedPackets.insert(ackNum);
                        inFlightPackets.erase(ackNum);
                        inFlightHeaders.erase(ackNum);
                        packetTimers.erase(ackNum);
                    
                        // Slide window forward only if the lowest unACKed packet was just acknowledged
                        while (ackedPackets.count(baseSeqNum)) {
                            ++baseSeqNum;
                        }
                    }
                }
        ...
        for (auto& [seq, sendTime] : packetTimers) {
            if (ackedPackets.count(seq)) continue;

            auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - sendTime).count();
            if (elapsed >= 500) {
                PacketHeader& header = inFlightHeaders[seq];
                std::vector<char>& data = inFlightPackets[seq];
                uint32_t length = ntohl(header.length);
                ...
                // Reset timer for the retransmitted packet
                packetTimers[seq] = std::chrono::steady_clock::now();
            }
        }
    }
